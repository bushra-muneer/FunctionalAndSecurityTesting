...
public class DrinkVendingMachine {
    ...
    public DrinkVendingMachine(float startCredit, BRANDS brand) {
        this.wallet = new Wallet(startCredit);
        this.availableCredit = this.wallet.getAllCredit(); // Assuming Wallet has a getAllCredit method
    ...
    public Snack selectDrink(int arrayPosition){
        if(arrayPosition >= 0 && arrayPosition < drinkList.size()) {
  //It was not satisfying the condition, so I changed it to the above condition
